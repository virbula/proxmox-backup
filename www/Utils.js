...
Ext.define('PBS.Utils', {
    ...
    cryptmap: ['none', 'mixed', 'sign-only', 'encrypt'],

    cryptText: [Proxmox.Utils.noText, gettext('Mixed'), gettext('Signed'), gettext('Encrypted')],

    cryptIconCls: ['', '', 'lock faded', 'lock good'],
    ...
    render_size_usage: function (val, max) {
        ...
        return (
            ((val * 100) / max).toFixed(2) +
            '% (' +
            Ext.String.format(
                gettext('{0} of {1}'),
                Proxmox.Utils.format_size(val),
                Proxmox.Utils.format_size(max),
            ) +
            ')'
        );
    },
    ...
    parse_snapshot_id: function (snapshot) {
        ...
        let namespaces = [];
        let nsPaths = [];
        snapshot = snapshot.replace(nsRegex, (_, nsPath, ns) => {
            nsPaths.push(nsPath);
            namespaces.push(ns);
            return '';
        });
    ...
    constructor: function () {
        ...
        Proxmox.Utils.override_task_descriptions({
            'acme-deactivate': (type, id) =>
                Ext.String.format(gettext('Deactivate {0} Account'), 'ACME') +
                ` '${id || 'default'}'`,
            'acme-register': (type, id) =>
                Ext.String.format(gettext('Register {0} Account'), 'ACME') +
                ` '${id || 'default'}'`,
            'acme-update': (type, id) =>
            ...
            'tape-backup': (type, id) =>
                PBS.Utils.render_tape_backup_id(id, gettext('Tape Backup')),
            'tape-backup-job': (type, id) =>
                PBS.Utils.render_tape_backup_id(id, gettext('Tape Backup Job')),
    ...
    render_tuning_options: function (tuning) {
        ...
        options.push(
            `${gettext('GC Access-Time Support Check')}: ${gc_atime_safety_check ?? true}`,
        );

        let gc_atime_cutoff = tuning['gc-atime-cutoff'];
        delete tuning['gc-atime-cutoff'];
        let gc_atime_cutoff_rendered = Proxmox.Utils.format_duration_human(
            (gc_atime_cutoff ?? 1445) * 60,
        );
