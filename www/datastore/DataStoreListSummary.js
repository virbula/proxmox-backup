...
Ext.define('PBS.datastore.DataStoreListSummary', {
    ...
    setStatus: function (statusData) {
            ...
            Proxmox.Utils.API2Request({
                ...
                success: (response) => {
                    ...
                    if (config['maintenance-mode']) {
                        const [_type, msg] = PBS.Utils.parseMaintenanceMode(
        ...
        let estimate = PBS.Utils.render_estimate(statusData['estimated-full-date'], null, {
            data: statusData,
        });
        ...
        vm.set(
            'deduplication',
            PBS.Utils.calculate_dedup_factor(statusData['gc-status']).toFixed(2),
        );
